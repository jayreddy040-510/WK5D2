...
def richer_than_england
  ...
  select name
  from countries
  where (gdp/population) > (select gdp/population from countries where name = 'United Kingdom') and continent = 'Europe'
  SQL
end
...
def population_constraint
  ...
  from countries
  where population between (select population from countries where name = 'Canada') and (select population from countries where name = 'Poland') and name != 'Canada' and name != 'Poland'
  SQL
end
